...
function [ Tmat,Sigmat, FiltSigmat ] = Ttest1Heatmap(sampleperiod,adjsigframe,filtth,alphath,timecolumn,electrodecolumn,Xtick,Xticklabel,Ytick,Yticklabel,Title,XLabel,YLabel,expdata1)
...
% Tmat is the matrix of p values over time and electrode
...
Tmat=zeros(tindexmax,electmax);
Sigmat=zeros(tindexmax,electmax);
FiltSigmat =zeros(tindexmax,electmax);

for i=1:1:tindexmax,
    for j=1:electmax,
        [h,p,ci,stats] = Ttest1Conditioned(conditioncolumn,sessioncolumn,subjectcolumn,alphath,i,j,expdata1);
        Tmat(i,j)=stats.tstat;
        if (stats.tstat>-2) && (stats.tstat<2)  ,
            Sigmat(i,j)=0;
        else
            Sigmat(i,j)=stats.tstat;
        end    
    end
end
...
%illusmap=(Tmat).'; if you want to show all the P-values without filtering
illusmap=Sigmat.';
% next line flip the columns to reflect the order requested i.e [D1:D32 C1:C32 E1:E32 G1:G32 B1:B32 F1:F32 A1:A32]
OrderedIllusmap=[illusmap(97:128,:); illusmap(65:96,:); illusmap(129:160,:); illusmap(193:224,:); illusmap(33:64,:); illusmap(161:192,:); illusmap(1:32,:)];% because illusmap was transposed of Tmat
